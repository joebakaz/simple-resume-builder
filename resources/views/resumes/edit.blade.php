            ...
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <div class="max-w-xl">
                    <section>
                        ...
                        <form method="post" action="{{ route('resumes.update', $resume) }}"  class="mt-6 space-y-6">
                            @csrf
                            @method('patch')
                            <x-input-label for="title" :value="__('Title')" />
                            <x-text-input id="title" name="title" type="text" class="mt-1 block w-full" :value="old('email', $resume->title)" required autofocus autocomplete="title" />
                            
                            <x-input-label for="about_me" :value="__('About Me')" />
                            <textarea name="about_me" id="about_me" class="mt-1 block w-full" required>{{ $resume->about_me }}</textarea>
                    ...
                    <section>
                        <header class="p-4 section-header">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-600">
                                    {{ __("Update your experience here.") }}
                                </p>
                            </div>
                            <div class="icon-buttons">
